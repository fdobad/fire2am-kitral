...
from .fire2am_utils import get_params, safe_cast_ok
...
class fire2amClassDialogArgparse(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        if any(g in self.args.keys() for g in self.groups):
            ...
            msg = (
                TAG
                + " ERROR:Group name (parser_group.title) in args.dest names:\n\t"
                + str(offenders)
    ...
    def setupUi(self):
        ...
        self.button_load = QtWidgets.QPushButton(
            self.style().standardIcon(QtWidgets.QStyle.StandardPixmap.SP_DialogOpenButton), "Load"
        )
        self.button_save = QtWidgets.QPushButton(
            self.style().standardIcon(QtWidgets.QStyle.StandardPixmap.SP_DialogSaveButton), "Save"
        )
    ...
    def slot_itemChanged(self, item, column):
        ...
        if self.parser[key]["type"] is str:
            ...
            # print(f"Item type {self.parser[key]['type']} not implemented!")
